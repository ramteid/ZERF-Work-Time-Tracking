...
static LANGUAGES: &[LangDef] = &[
    LangDef {
        ...
        translations: &[
            ...
            ("report_upload_pre_start_review_body", "User {user_id} ({first_name} {last_name}) has current start date {start_date} inside or after period {period}, and a start-date change queued this period for review. Correct the start date or historical rows before retrying the timesheet PDF export."),
            ("report_upload_pre_start_content_body", "User {user_id} ({first_name} {last_name}) has current start date {start_date} inside or after period {period}, and stored report rows exist before the current start date. Correct the start date or historical rows before retrying the timesheet PDF export."),
            ("report_upload_unsettled_time_body", "User {user_id} ({first_name} {last_name}) is archived or has time tracking disabled, but period {period} still contains draft, submitted, or unresolved rejected time entries. Resolve those rows before retrying the timesheet PDF export."),
    ...
    LangDef {
        ...
        translations: &[
            ...
            ("report_upload_pre_start_review_body", "Benutzer {user_id} ({first_name} {last_name}) hat aktuell das Startdatum {start_date}, das innerhalb oder nach der Periode {period} liegt, und eine \u{00c4}nderung des Startdatums hat diese Periode zur Pr\u{00fc}fung vorgemerkt. Bitte korrigieren Sie das Startdatum oder die historischen Daten, bevor der Stundenzettel-PDF-Export erneut versucht wird."),
            ("report_upload_pre_start_content_body", "Benutzer {user_id} ({first_name} {last_name}) hat aktuell das Startdatum {start_date}, das innerhalb oder nach der Periode {period} liegt, und es existieren gespeicherte Berichtsdaten vor dem aktuellen Startdatum. Bitte korrigieren Sie das Startdatum oder die historischen Daten, bevor der Stundenzettel-PDF-Export erneut versucht wird."),
            ("report_upload_unsettled_time_body", "Benutzer {user_id} ({first_name} {last_name}) ist archiviert oder hat die Zeiterfassung deaktiviert, aber die Periode {period} enth\u{00e4}lt noch Zeiteintr\u{00e4}ge im Status Entwurf, eingereicht oder ungel\u{00f6}st abgelehnt. Bitte l\u{00f6}sen Sie diese Eintr\u{00e4}ge auf, bevor der Stundenzettel-PDF-Export erneut versucht wird."),
