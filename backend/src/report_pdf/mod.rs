//! Server-side PDF rendering primitives and layouts.
//!
//! Pure rendering logic with no database access (mirrors `email.rs` in that
//! respect). This module owns the shared machinery — page geometry, palette,
//! fonts, the table [`Renderer`] and PDF assembly — using `pdf-writer`'s
//! content-stream operators with the PDF standard Type 1 fonts (Helvetica /
//! Helvetica-Bold). Built-in fonts require no font files to bundle, which keeps
//! the binary small and avoids font-licensing questions.
//!
//! The concrete documents live in the submodules and only describe their own
//! column layout and row content:
//!   * `timesheet` — per-employee monthly timesheet ([`render_timesheet_pdf`])
//!   * `payroll` — company-wide monthly payroll report ([`render_payroll_report_pdf`])
//!
...
//!
//! Right-/center-aligned columns contain only digits, `:`, `+`, `-`, `.`, `,`
//! and spaces, so [`glyph_width_1000`] hardcodes the public-domain Adobe AFM
...
///
/// The active column layout is swappable ([`Renderer::set_columns`]) so one
/// document can contain several tables with different shapes.
struct Renderer<'a> {
    ...
    y: f32,
    language: &'a Language,
    /// Column layout the table header and rows are drawn against.
    columns: &'a [Column],
}

impl<'a> Renderer<'a> {
    fn new(language: &'a Language, columns: &'a [Column]) -> Self {
        Self {
            pages: Vec::new(),
            content: Content::new(),
            y: MARGIN_TOP_MM,
    ...
    /// Switch to a different column layout for the tables drawn from here on.
    fn set_columns(&mut self, columns: &'a [Column]) {
    ...
    /// Draw a document title with a secondary line below it, then advance past
    /// the whole block.
    fn draw_title_block(&mut self, title: &str, subtitle: &str) {
        self.draw_text(
            title,
            MARGIN_LEFT_MM,
            self.y + 6.0,
    ...
    /// Draw a bold section heading above a table and advance past it.
    fn draw_section_heading(&mut self, heading: &str) {
        // Breathing room after whatever came before — but not when the section
        // is the first thing on a page.
        if self.y > MARGIN_TOP_MM {
            self.y += 5.0;
        }
        // Keep the heading together with its table header and at least one row
        // so a section never starts at the very bottom of a page.
        self.ensure_space(8.0 + HEADER_HEIGHT_MM + ROW_HEIGHT_MM, false);
        self.draw_text(
            heading,
            MARGIN_LEFT_MM,
            self.y + 5.0,
    ...
    fn draw_table_header(&mut self) {
        ...
        // Resolve positions and labels first: drawing borrows `self` mutably
        // while `columns`/`column_x` borrow it immutably.
        let labels: Vec<(f32, String)> = self
            ...
            .map(|(index, column)| {
                (
                    ...
                    self.column_x(index) + 1.0,
                    i18n::translate(self.language, column.header_key, &[]),
                )
            })
            .collect();
    ...
    /// Draw a highlighted total row: bold label at the left margin, bold values
    /// aligned under their own columns.
    fn draw_total_row(&mut self, label: &str, cells: &[(usize, String)]) {
        self.ensure_space(ROW_HEIGHT_MM, true);
        self.fill_rect(
            MARGIN_LEFT_MM,
            self.y,
        ...
        for (column_index, text) in cells {
            let x = self.aligned_x(*column_index, text, 7.5);
            self.draw_text(text, x, baseline, PdfFont::Bold, 7.5, TITLE_COLOR);
        }
        self.y += ROW_HEIGHT_MM;
    }

    /// Draw a label/value summary line (e.g. a flextime balance), with the
    /// value aligned under `value_column`. Unlike data rows this does not
    /// redraw the table header on overflow — it sits below the table, just like
    /// in the old browser-side renderer.
    fn draw_summary_row(&mut self, label: &str, value: &str, value_column: usize) {
        ...
        let value_x = self.aligned_x(value_column, value, 7.5);
        self.draw_text(
            value,
            value_x,
            baseline,
    ...
    /// Draw a plain, non-tabular note line (e.g. "no entries in this period").
    fn draw_note(&mut self, text: &str) {
        self.ensure_space(ROW_HEIGHT_MM, false);
        let baseline = self.y + 3.8;
        self.draw_text(
            text,
...
/// [`format_minutes`] / [`format_signed_minutes`], `HH:MM` time strings and
/// decimal numbers can ever produce — see the module docs for why that is
/// sufficient here.
...
mod tests {
    ...
    #[test]
    fn encode_winansi_maps_latin1_and_replaces_everything_else() {
        assert_eq!(encode_winansi("AZ"), vec![b'A', b'Z']);
        // German umlauts live in the Latin-1 supplement and map 1:1.
        assert_eq!(encode_winansi("\u{00fc}"), vec![0xFC]);
        // Outside Latin-1 (e.g. an em dash) becomes '?'.
        assert_eq!(encode_winansi("\u{2014}"), vec![b'?']);
    }
}
